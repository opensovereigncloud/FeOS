use std::{collections::HashMap, num::TryFromIntError, os::unix::net::UnixStream, path::{Path, PathBuf}, process::{Child, Command}, sync::Mutex, thread::sleep, time};
use log::info;
use uuid::Uuid;
use serde_json::json;
...
impl  Manager {
    ...
    pub fn init_vmm(&self, id: Uuid, wait: bool) -> Result<(), Error> {
        ...
        let vm = Command::new(self.ch_bin.clone())
            .arg("--api-socket")
            .arg(id.to_string()).spawn().map_err(Error::CHCommandFailure)?;
        ...
        let tries = 3;
        for i in 0..tries {
            info!("waiting until socket is open: vmm: {}, tries: {}/{}", id.to_string(), i+1, tries);
    ...
    pub fn create_vm(&self, id: Uuid, cpu: u32, memory: u64, root_fs: PathBuf) -> Result<(), Error> {
        ...
        let response = api_client::simple_api_full_command_and_response(&mut socket, "PUT", "vm.create", Some(&cfg.to_string())).map_err(Error::CHApiFailure)?;
        if response.is_some() {
            info!("create vm: id {}, response: {}", id.to_string(), response.unwrap())
        }
    ...
    pub fn boot_vm(&self, id: Uuid) -> Result<(), Error> {
        ...
        let mut socket = UnixStream::connect(id.to_string()).map_err(Error::SocketFailure)?;
        let response = api_client::simple_api_full_command_and_response(&mut socket, "PUT", "vm.boot", None).map_err(Error::CHApiFailure)?;
        if response.is_some() {
            info!("boot vm: id {}, response: {}", id.to_string(), response.unwrap())
        }
    ...
    pub fn ping_vmm(&self, id: Uuid) -> Result<(), Error> {
        ...
        let mut socket = UnixStream::connect(id.to_string()).map_err(Error::SocketFailure)?;
        let response = api_client::simple_api_full_command_and_response(&mut socket, "GET", "vmm.ping", None).map_err(Error::CHApiFailure)?;
        if response.is_some() {
            info!("ping vmm: id {}, response: {}", id.to_string(), response.unwrap());
        }
    ...
    pub fn get_vm(&self, id: Uuid) -> Result<String, Error> {
        ...
        let mut socket = UnixStream::connect(id.to_string()).map_err(Error::SocketFailure)?;
        let response = api_client::simple_api_full_command_and_response(&mut socket, "GET", "vm.info", None).map_err(Error::CHApiFailure)?;
